# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .retrieve_testset_response_column_definitions_item import RetrieveTestsetResponseColumnDefinitionsItem


class RetrieveTestsetResponse(UniversalBaseModel):
    id: str = pydantic.Field()
    """
    Public testset ID.
    """

    name: str = pydantic.Field()
    """
    Testset name.
    """

    created_at: dt.datetime = pydantic.Field()
    """
    When the testset was created.
    """

    updated_at: dt.datetime = pydantic.Field()
    """
    When the testset was last updated.
    """

    current_max_row_index: float = pydantic.Field()
    """
    Highest row index currently present in the testset.
    """

    project: typing.Optional[str] = pydantic.Field(default=None)
    """
    Project identifier when the testset is associated with a project.
    """

    column_definitions: typing.Optional[typing.List[RetrieveTestsetResponseColumnDefinitionsItem]] = pydantic.Field(
        default=None
    )
    """
    Configured columns for testset rows.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
