# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .update_experiment_response_workflow_item import UpdateExperimentResponseWorkflowItem


class UpdateExperimentResponse(UniversalBaseModel):
    id: str = pydantic.Field()
    """
    Experiment ID.
    """

    name: str = pydantic.Field()
    """
    Experiment name.
    """

    description: typing.Optional[str] = pydantic.Field(default=None)
    """
    Experiment description.
    """

    dataset: typing.Optional[str] = pydantic.Field(default=None)
    """
    Dataset ID associated with the experiment.
    """

    dataset_id: typing.Optional[str] = pydantic.Field(default=None)
    """
    Dataset ID associated with the experiment.
    """

    dataset_name: typing.Optional[str] = pydantic.Field(default=None)
    """
    Dataset name, when available.
    """

    workflow_count: typing.Optional[int] = pydantic.Field(default=None)
    """
    Number of workflow steps.
    """

    status: str = pydantic.Field()
    """
    Experiment execution status.
    """

    progress: typing.Optional[float] = pydantic.Field(default=None)
    """
    Execution progress percentage.
    """

    created_at: dt.datetime
    started_at: typing.Optional[dt.datetime] = None
    completed_at: typing.Optional[dt.datetime] = None
    tags: typing.Optional[typing.List[typing.Dict[str, typing.Any]]] = pydantic.Field(default=None)
    """
    Tags attached to the experiment.
    """

    workflow: typing.Optional[typing.List[UpdateExperimentResponseWorkflowItem]] = pydantic.Field(default=None)
    """
    Workflow tasks configured for the experiment.
    """

    evaluator_ids: typing.Optional[typing.List[str]] = pydantic.Field(default=None)
    """
    Evaluator IDs used for scoring.
    """

    evaluator_slugs: typing.Optional[typing.List[str]] = pydantic.Field(default=None)
    """
    Backward-compatible evaluator identifiers stored by the backend.
    """

    evaluator_workflow_ids: typing.Optional[typing.List[str]] = pydantic.Field(default=None)
    """
    Eval-only workflow versions used for scoring.
    """

    batch_size: typing.Optional[int] = None
    concurrency: typing.Optional[int] = None
    enable_tracing: typing.Optional[bool] = None
    error_message: typing.Optional[str] = pydantic.Field(default=None)
    """
    Failure details when status is failed.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
